...
import {EditButton, DeleteButton, ToTaskButton, CheckTimeButton, NewCheck, NewTask} from 'components/Buttons/TaskControlButton';
...
class EventsOperationsLogic extends Component {
    ...
    componentDidMount() {
        ...
        this.changeCallback = EmptyFunc;
        this.addCallback = EmptyFunc;
    }
    ...
    eventClickHandler = (event) => {
        this.setState({
            ...
            currentEvent: event,
            selectedTaskId: event.event.extendedProps.taskId
        });
    };
    ...
    changeEventModal = (event) => {
        ...
        params = {...eventApi.extendedProps, title: eventApi.title, time: getMinutesBetweenDates(startTime, endTime)};
    ...
    copyEvent = (event) => {
        const eventApi = event.event;
        let params = {};

        params.start = eventApi.start;
        params.end = eventApi.end;
        params.title = eventApi.title;
        params.time = getMinutesBetweenDates(params.start, params.end);

        params = {...params, ...eventApi.extendedProps};

        this.changeCallback = async () => {
            await this.setState({currentApi: event.calendarApi});
            await this.addEvent(params);
            this.changeCallback = EmptyFunc;
        };
    };

    onLeaveCalendar = async (event) => {
            ...
            <React.Fragment>
                      ...
                      return this.props.zones.map(item => {
                        return <React.Fragment key={item.id}>
                          ...
                          <CalendarController
                              ...
                              onLeaveCalendar={this.onLeaveCalendar}
                              eventCopy={this.copyEvent}
                ...
                <FixBottomContainer show={this.state.onSelectPanelShow} elementSelector={'.fc-highlight'}>
                    <ButtonGroup variant="contained" size="small" aria-label="small outlined button group">
                        <NewTask onClick={this.addEventModal.bind(null, 'task')}>&nbsp;Бронь</NewTask>
                        <NewCheck onClick={this.addEventModal.bind(null, 'time')}>&nbsp;Отметка о времени</NewCheck>
                    </ButtonGroup>
                </FixBottomContainer>
                <FixBottomContainer show={this.state.onPickEventPanelShow} elementSelector={'.fc-allow-mouse-resize'}>
                    <ButtonGroup variant="contained" size="small" aria-label="small outlined button group">
                        <EditButton label="Редактировать задачу" onClick={this.changeEventModal}/>
                        <DeleteButton label="Удалить задачу" onClick={this.removeHandler}/>
                        <CheckTimeButton label="Отметить время по задаче" onClick={this.checkTime}/>
                        <ToTaskButton label="Перейти к задаче" taskId={this.state.selectedTaskId}/>
                    </ButtonGroup>
                </FixBottomContainer>
            </React.Fragment>
        );
