...
const Day = observer(class DayClass extends Component {
    ...
    addItem = (event) => {
        ...
        if(this.props.appUi) {
            ...
            this.props.appUi.modal.setParams({date: this.props.date, zone: this.props.zone});
    ...
    render() {
        return (
            <div onMouseUp={this.changePlanItem.bind(null, {date: this.props.date, zone: this.props.zone})} className={'day'}>
                <div className={`day__title ${this.props.active ? 'active' : ''}`}>
                    {this.props.date}
                </div>
                <div className={'day__body'}>
                    {this.showPlanItems()}
                    <Grid height={630} step={30}/>
